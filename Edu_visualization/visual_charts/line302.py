...
xaxis_data = data.loc[(data['课程号'].map(str) == '1') & (data['班级'].map(str) == '302')]["学号"].map(str).tolist()
yaxis_data1 = data.loc[(data['课程号'].map(str) == '1') & (data['班级'].map(str) == '302')]["分数"].tolist()
yaxis_data2 = data.loc[(data['课程号'].map(str) == '2') & (data['班级'].map(str) == '302')]["分数"].tolist()
yaxis_data3 = data.loc[(data['课程号'].map(str) == '3') & (data['班级'].map(str) == '302')]["分数"].tolist()
...
def line():
    chart_line = (
        ...
        .set_global_opts(
            ...
            title_opts=opts.TitleOpts(title="302班各学科学生成绩", title_link="#"),
...
chartLine = line()
chartLine.render("../../templates/visual/pyecharts_visual/line302.html")
